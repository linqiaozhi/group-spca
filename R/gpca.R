#' @title  Sparse Principal Component Analysis (gpca).
#
...
#' @export
gpca <- function(X, k=NULL, alpha=1e-4, beta=1e-4, center=TRUE, scale=FALSE, max_iter=1000, tol=1e-5, verbose=TRUE, group_membership = NULL) UseMethod("gpca")

#' @export
gpca.default <- function(X, k=NULL, alpha=1e-4, beta=1e-4, center=TRUE, scale=FALSE, max_iter=1000, tol=1e-5, verbose=TRUE, group_membership = NULL) {
  ...
  while (noi <= max_iter && improvement > tol) {
        ...
        #  psi
        ################################
        num_groups  <-  max(group_membership)
        for (i in 1:num_groups) {
            xx  <- B_temp[group_membership == i]
            xx_2  <- sqrt(sum(xx^2))
            if ( xx_2 > kappa) {
                #print(i)
                #print(xx)
